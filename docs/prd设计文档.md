...
2. 用30分钟骑行（电动车或者摩托车之类的）等时圈作为边界约束
3. 取两者交集作为最终服务范围
4. 简化多边形为8-12个顶点(最小必须8个点)
...
| 任务 | 时间 | 优先级 |
|------|------|--------|
| 项目初始化 (Vue3 + Vite + TailwindCSS) | 1h | ✅ 已完成 |
| 主布局实现 (地图区 + 侧边栏) | 1h | P0 |
| 高德地图集成与初始化 | 2h | P0 |
| 门店标记点展示 | 2h | P0 |
| 门店详情卡片弹窗 | 1h | P0 |
...
1. **高德地图Key**：需要在 [高德开放平台](https://lbs.amap.com/) 申请Web端Key，替换代码中的 `YOUR_AMAP_KEY`

2. **数据说明**：当前使用模拟数据，订单分布遵循距离衰减模型，超时原因按真实业务比例分配
